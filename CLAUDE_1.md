...
- **Routing:** routed page per project writeup via `react-router-dom`
  (HashRouter default on GitHub Pages).

Still open (ask when relevant, not before):
- Exact repo name (needed for the Vite `base` setting).
- HashRouter (default) vs. BrowserRouter + 404 workaround for clean URLs.
- Whether to auto-create the GitHub Actions deploy workflow.
